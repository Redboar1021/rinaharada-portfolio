...
### C. 管理画面 (`/admin`)
- **認証**: `@aws-amplify/ui-react` の `<Authenticator>` コンポーネントを使用し、実装工数を削減すること。
- **レイアウト**: ログイン後、ダッシュボードを表示。左側または上部にナビゲーション（「スケジュール管理」「ビデオ管理」「ログアウト」）を配置。
- **ルーティング**:
  - `/admin/schedule`: スケジュール一覧・追加・編集
  - `/admin/videos`: ビデオ一覧・追加・編集

#### 1. スケジュール管理機能
- **一覧表示**: 日付順に並べたテーブル表示。各行に「編集」「削除」ボタン。
- **新規追加 / 編集フォーム**:
  - モーダルまたは別ページで実装。
  - **入力項目**:
    - 日付 (`type="date"`)
    ...
    - 場所 (Text)
    - 詳細 (Textarea)
    - 画像 (File Input): Amplify Storage (`Store.put`) を使用してS3へアップロードし、取得したキー(key)またはURLを保存。
- **バリデーション**: 必須項目（日付、タイトル）のチェック。

#### 2. ビデオ管理機能
- **一覧表示**: 登録済み動画のサムネイルとタイトルをリスト表示。ドラッグ&ドロップ等で並び替えできると尚良いが、まずは「表示順（数値）」の直接入力でも可。
- **新規追加 / 編集フォーム**:
  - **入力項目**:
    - 動画タイトル (Text)
    - YouTube URL (Text): `https://youtu.be/...` 等の短縮URLも許容し、保存時に動画IDを抽出して保存するロジックを実装推奨。
    - 表示順序 (Number)
